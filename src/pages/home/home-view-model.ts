...
export class HomeViewModel extends BaseViewModel {
  ...
  async fetchAllClasses(){
    ...
    const response: SingleClass[]|HttpError =  await httpService.sendGet('/class', httpService.getBearerToken())
    this.stopLoading();
  ...
  updateClassList = (response:SingleClass[])=>{
    ...
    response.map(item=>{
      const{email,firstName, lastName, profilePictureUrl,defaultProfilePictureHex} = item.mainTeacher;
