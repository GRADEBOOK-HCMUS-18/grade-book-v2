...
class FileService{
    ...
    readFile(event:any):Array<Array<string>>{
        if(event ===undefined)
            return[];
        ...
        if(this.dictionary.has(fileType))
        {
            ...
            const data:Array<Array<string>>=service?.readFile(event);
            return data;
        }
        return [];
    }

    writeFile(headers:Array<string>, content :Array<Array<string>>,fileName:string,fileType:string){
        if(this.dictionary.has(fileType))
        {
            const service = this.dictionary.get(fileType);
...
class ExcelFileService{
    ...
    writeFile(headers:string[],content:Array<Array<string>>, fileName:string ):void{
    ...
    readFile(event:any):Promise<Array<Array<string>> > {
        ...
        return new Promise((resolve,reject)=>{
            reader.onload = (event:any)=>{
                ...
                const array_data: Array<Array<string>>= CSVService.csvToArray(csv_data);
